...
class ConvNext_GCC_Block(nn.Module):
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6, global_kernel_size=14, use_pe=True):
        super().__init__()
        self.gcc_H = GCC_conv(dim//2, 'H', global_kernel_size, use_pe)
        self.gcc_W = GCC_conv(dim//2, 'W', global_kernel_size, use_pe)
        ...
        for i, [depth, dim, gcc_bs_index, input_size] in enumerate(zip(depths, dims, gcc_bs_indices, stages_rs)):
            ...
            for j in range(depth):
                if j<gcc_bs_index:
                    ...
                    blocks.append(ConvNext_GCC_Block(dim=dim, drop_path=dp_rates[cur + j],
                                                     layer_scale_init_value=layer_scale_init_value,
...
@register_model
def convnext_gcc_xt(pretrained=False, in_22k=False, **kwargs):
    """
    ...
    model = ConvNeXt_gcc(depths=[3, 3, 9, 3], dims=[48, 96, 192, 384], gcc_bs_indices=[3, 3, 6, 2], stages_rs=[56, 28, 14, 7], **kwargs)
    if pretrained:
...
@register_model
def convnext_gcc_t(pretrained=False, in_22k=False, **kwargs):
    model = ConvNeXt_gcc(depths=[3, 9, 36, 6], dims=[56, 112, 224, 448], gcc_bs_indices=[3, 9, 24, 4], stages_rs=[56, 28, 14, 7], **kwargs)
    if pretrained:
        raise NotImplementedError("no pretrained model")
    # test
    input = torch.randn(2, 3, 224, 224)
