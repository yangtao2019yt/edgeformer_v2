...
class ConvNext_GCC_gl_Block(nn.Module):
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6, global_kernel_size=14, use_pe=True):
        super().__init__()
        self.pwconv0 = nn.Conv2d(dim, dim//2, kernel_size=1, padding=0, groups=dim//2)
        self.gcc_H = GCC_conv(dim//4, 'H', global_kernel_size, use_pe)
        self.gcc_W = GCC_conv(dim//4, 'W', global_kernel_size, use_pe)
        self.l_conv = nn.Conv2d(dim // 2, dim // 2, kernel_size=7, padding=3, groups=dim//2)
    ...
    def forward(self, x):
        ...
        x = torch.cat((x, self.l_conv(x)), dim=1)
...
class ConvNeXt_gcc_gl(nn.Module):
    def __init__(self, in_chans=3, num_classes=1000,
                 depths=[3, 3, 9, 3], dims=[96, 192, 384, 768], drop_path_rate=0.,
                 layer_scale_init_value=1e-6, head_init_scale=1.,
                 gcc_bs_indices=[3, 3, 6, 2], stages_rs=[56, 28, 14, 7] # gcc block start indices, input resolutions of four stages
                 ):
        super().__init__()
        ...
        for i, [depth, dim, gcc_bs_index, input_size] in enumerate(zip(depths, dims, gcc_bs_indices, stages_rs)):
            ...
            for j in range(depth):
                if j<gcc_bs_index:
                    ...
                    blocks.append(ConvNext_GCC_gl_Block(dim=dim, drop_path=dp_rates[cur + j],
                                                     layer_scale_init_value=layer_scale_init_value,
...
@register_model
def convnext_gcc_gl_xt(pretrained=False, in_22k=False, **kwargs):
    """
    ...
    model = ConvNeXt_gcc_gl(depths=[3, 3, 9, 3], dims=[48, 96, 192, 384], gcc_bs_indices=[3, 3, 6, 2], stages_rs=[56, 28, 14, 7], **kwargs)
    if pretrained:
