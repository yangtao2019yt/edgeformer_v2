...
class dygcc_dype_v1_Block(nn.Module):
    def __init__(self,
        dim,
        drop_path=0.,
        ...
        self.use_pe = use_pe
        self.dim = dim
        self.pe_gen = gen(dim) if use_pe else None
...
class ConvNeXt_dygcc_dype_v1(nn.Module):
    def __init__(self, in_chans=3, num_classes=1000, 
        ...
        for i in range(4):
            if i < 2:   # for stage 0 and 1, no gcc
                ...
                stage = nn.Sequential(*[
                    dygcc_dype_v1_Block(dim=dims[i], drop_path=dp_rates[cur + j], layer_scale_init_value=layer_scale_init_value,
...
@register_model
def convnext_dygcc_dype_v1_tt(pretrained=False, in_22k=False, **kwargs):
    model = ConvNeXt_dygcc_dype_v1(depths=[3, 3, 9, 3], dims=[48, 96, 192, 384], **kwargs)
