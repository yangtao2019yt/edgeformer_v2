...
class dygcc_dype_v5_Block(nn.Module):
    def __init__(self,
        dim,
        drop_path=0.,
    ...
    def forward(self, x):
        ...
        HW_weight = torch.fft.ifftshift(HW_weight, dim=[2,3])
...
class ConvNeXt_dygcc_dype_v5(nn.Module):
    def __init__(self, in_chans=3, num_classes=1000, 
        ...
        for i in range(4):
            if i < 2:   # for stage 0 and 1, no gcc
                ...
                stage = nn.Sequential(*[
                    dygcc_dype_v5_Block(dim=dims[i], drop_path=dp_rates[cur + j], layer_scale_init_value=layer_scale_init_value,
...
@register_model
def convnext_dygcc_dype_v5_tt(pretrained=False, in_22k=False, **kwargs):
    model = ConvNeXt_dygcc_dype_v5(depths=[3, 3, 9, 3], dims=[48, 96, 192, 384], **kwargs)
